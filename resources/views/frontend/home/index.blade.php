...
<div id="content" class="content">
    <section class="box-banner">
        <div class="owl-carousel">
            ...
            <a href="{{ $banner_detail->url ?? '#' }}" target="_blank">
                <div class="item" style="background-color: #e2ddb7;">
                    <div class="img-banner banner-portrait"
                        style="background-image: url('{{ $banner_detail->slide_banner_mobile }}')"></div>
                    <div class="img-banner" style="background-image: url('{{ $banner_detail->slide_banner_pc }}')">
                    </div>
                </div>
            </a>
            @break
            ...
            <div class="item" style="background-color: #e2ddb7;">
                <iframe
                    src="https://www.youtube.com/embed/{{$banner_detail->url}}?mute=1&autoplay=1&loop=1&playlist={{$banner_detail->url}}"
                    frameborder="0"></iframe>
                <a href="https://www.youtube.com/embed/{{$banner_detail->url}}?autoplay=1&loop=1&playlist={{$banner_detail->url}}"
                    target="_blank">
                    <div class="this-none"></div>
                </a>
            </div>
            ...
            <div class="row box-List lazyload">
                ...
                <div class="card col-xl-2 col-lg-3 col-sm-4 col-6 list">
                    <div class="btn-heart {{ $recommended_product->favorites_count > 0 ? 'active' : '' }}"
                        data-fav="{{ $recommended_product->favorites_count }}"
                        data-product="{{ $recommended_product->id }}"></div>
                    <div class="card-body">
                        <a
                    ...
                    <div class="card-footer">
                        ...
                        <button type="button" class="btn btn-cart w-100"
                            {{ (empty(Auth::user()) or GlobalFn::productReservedOnCart($recommended_product->id) or GlobalFn::productOutOfStock($recommended_product->id)) ? 'disabled' : '' }}
                            data-product="{{ $recommended_product->id }}">{{ GlobalFn::productOutOfStock($recommended_product->id) ? __('messages.sold_out') : (GlobalFn::productReservedOnCart($recommended_product->id) ? __('messages.out_of_stock') : __('messages.add_basket')) }}</button>
                    </div>
                </div>
                @endforeach
            </div>
