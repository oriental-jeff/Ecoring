      ...
      <div class="col-xl-2 col-lg-12 d-flex pl-0">
        ...
        <button class="navbar-toggler" type="button">
          <div class="position-relative d-inline-flex">
			<a href="../cart" id="boxOfProductmobile">
                  ...
                  <ul class="navbar-nav row">
                    @foreach($main['menus'] as $key => $menu)
                      ...
                      <li id="{{ $menu->id }}" class="nav-item {{ collect($menu->childrens)->count() > 0 ? $parent_menu_active : $menu_active }} col">
                        <a href="{{ collect($menu->childrens)->count() > 0 ? '#' : route($menu->pages->route_name, ['locale' => get_lang()]) }}" class="nav-link {{ collect($menu->childrens)->count() > 0 ? 'dropdown-toggle' : '' }}"  data-toggle="{{ collect($menu->childrens)->count() > 0 ? 'dropdown' : '' }}"><img src="{{ asset('images/' . $menu->icon) }}" class="icon-menu">{{ $menu->{ get_lang('name')} }}</a>
