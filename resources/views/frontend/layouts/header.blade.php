                ...
                <div class="collapse navbar-collapse" id="navbarCollapse">
                    <div class="row w-100 m-0">
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                    <ul class="navbar-nav row">
                                        ...
                                        @endif
                                        <li id="{{ $menu->id }}"
                                            class="nav-item {{ collect($menu->childrens)->count() > 0 ? $parent_menu_active : $menu_active }} col">
                                            <a href="{{ collect($menu->childrens)->count() > 0 ? '#' : route($menu->pages->route_name, ['locale' => get_lang()]) }}"
                                                class="nav-link {{ collect($menu->childrens)->count() > 0 ? 'dropdown-toggle' : '' }}"
                                                data-toggle="{{ collect($menu->childrens)->count() > 0 ? 'dropdown' : '' }}">
                                                <img src="{{ asset('images/' . $menu->icon) }}"
                                                    class="icon-menu">{{ $menu->{ get_lang('name')} }}
                                            </a>
                                            @if(collect($menu->childrens)->count() > 0)
                                            <div class="dropdown-menu">
                                                @foreach ($menu->childrens as $key => $child)
                                                <a href="{{ collect($child->childrens)->count() > 0 ? '#' : route($child->pages->route_name, ['locale' => get_lang()]) }}"
                                                    id="{{ $child->id }}"
                                                    class="dropdown-item {{ $child->page_id == $pages->id ? 'active' : '' }}">{{ $child->{ get_lang('name') } }}
                                                </a>
                                                @endforeach
                                            </div>
                                            @endif
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 border-left">
                                            ...
                                            <a href="{{ route('frontend.auth.login.form', ['locale' => get_lang()]) }}">
                                                <img src="{{ asset('images/I-login.svg') }}"
                                                    class="icon-login">{{ __('messages.login') }}
                                            </a>
                                            @endguest
                                            @auth
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                                <a class="dropdown-item"
                                                    href="{{ route('frontend.user.profile', ['locale' => get_lang()]) }}">{{ __('messages.data_profile') }}</a>
                                                <a class="dropdown-item"
                                                    href="{{ route('frontend.user.edit-password', ['locale' => get_lang()]) }}">{{ __('messages.edit_password') }}</a>
                                                <a class="dropdown-item" href="javascript:;"
                                                    onclick="event.preventDefault();document.getElementById('logout-form').submit();">{{ __('messages.logout') }}</a>
                                                <form id="logout-form"
                                                    action="{{ route('frontend.auth.logout', ['locale' => get_lang()]) }}"
                                        ...
                                        <div class="col-md-3 border-left">
                                            <div class="position-relative d-inline-flex">
                                                <a href="{{ route('frontend.cart', ['locale' => get_lang()]) }}"
                                                    id="boxOfProduct">
                        ...
                        <div class="mt-2 mt-md-0 col-12 pr-0" id="menuCategory">
                            <form action="{{ route('frontend.product', ['locale' => get_lang()]) }}" class="form-inline"
                                method="get">
                                @method('get')
                                @csrf
                            ...
                            <div class="menu-category">
                                <div class="owl-carousel">
                                    ...
                                    <div class="item">
                                        <a
                                            href="{{ route('frontend.product', ['locale' => get_lang(), 'category_id' => $main_category->id]) }}">{{ $main_category->{ get_lang('name') } }}</a>
                                    </div>
                                    @endforeach
                                </div>
                            </div>
    ...
    <nav class="navbar navbar-expand-xl navbar-light">
        <div class="collapse navbar-collapse" id="navbarNav">
            <form class="mt-2 mt-md-0 mb-0 col-12 border-bottom">
                <div class="box-Search form-inline">
                    <input class="form-control" type="text" placeholder="{{ __('messages.search') }}.."
                        aria-label="Search">
                    <button class="btn btn-outline-success my-2 my-0" type="submit"><i
                            class="fa fa-search"></i></button>
                </div>
            </form>
            <ul class="navbar-nav px-3 text-center">
                ...
                @endif
                <li id="{{ $menu->id }}"
                    class="nav-item {{ collect($menu->childrens)->count() > 0 ? $parent_menu_active : $menu_active }}">
                    <a href="{{ collect($menu->childrens)->count() > 0 ? '#' : route($menu->pages->route_name, ['locale' => get_lang()]) }}"
                        class="nav-link {{ collect($menu->childrens)->count() > 0 ? 'dropdown-toggle' : '' }}"
                        data-toggle="{{ collect($menu->childrens)->count() > 0 ? 'dropdown' : '' }}">
                        <img src="{{ asset('images/' . $menu->icon) }}"
                    ...
                    <div class="dropdown-menu">
                        ...
                        <a href="{{ collect($child->childrens)->count() > 0 ? '#' : route($child->pages->route_name, ['locale' => get_lang()]) }}"
                            id="{{ $child->id }}"
                            class="dropdown-item {{ $child->page_id == $pages->id ? 'active' : '' }}">{{ $child->{ get_lang('name') } }}</a>
                        </a>
                        @endforeach
                    </div>
                    @endif
            ...
            <div class="navbar-top-mobile">
                <div class="container">
                    <span class="my-3 mx-auto w-100">โทร : 02-118-6096 <span class="Nshow-p">|</span> <br
                            class="show-p"> 日本語 : 063-203-9807 (9:30-18:30)</span>
