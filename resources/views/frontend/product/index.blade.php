            ...
            <div class="row">
                <div class="col-lg-3">
                    <div class="b-sticky d-none d-lg-block">
                        <form id="form_search_product" class="form-Search"
                            action="{{ route('frontend.product', ['locale' => get_lang()]) }}" method="post">
                            @method('get')
                            @csrf
                            <div class="form-inline">
                                <input type="text" class="form-control" name="keyword"
                                    placeholder="{{ __('messages.search_products') }}.." aria-label="Search"
                            ...
                            <div class="box-Toggle" style="display: none;">
                                ...
                                <label for="category_checkbox{{ $category->id }}" class="d-block">
                                    <input type="checkbox" id="category_checkbox{{ $category->id }}"
                                        name="category_selected[]" value="{{ $category->id }}"
                                        {{ (!empty(request()->category_selected) and in_array($category->id, request()->category_selected)) ? 'checked' : '' }}>
                            ...
                            <button type="button" id="button-Toggle1" class="p-2 mt-2 border-0 w-100"
                                onclick="$('.box-Toggle').slideToggle();$(this).hide();$('#button-Toggle2').show();">
                            ...
                            <button type="button" id="button-Toggle2" class="p-2 mt-2 border-0 w-100"
                                style="display: none"
                                onclick="$('.box-Toggle').slideToggle();$(this).hide();$('#button-Toggle1').show();">
                            ...
                            <label for="grade_checkbox{{ $grade->id }}" class="d-block">
                                <input type="checkbox" id="grade_checkbox{{ $grade->id }}" name="grade_selected[]"
                                    value="{{ $grade->id }}"
                                    {{ (!empty(request()->grade_selected) and in_array($grade->id, request()->grade_selected)) ? 'checked' : '' }}>
                                {{ $grade->{ get_lang('name') } }}
                ...
                <div class="col-lg-9">
                    <div class="row border-bottom pb-1">
                        <div class="col-lg-7">
                            <h5 class="my-1">{{ __('messages.product') }} <span
                                    style="font-size: 0.8em;">({{ count($products) }} {{ __('messages.list') }})</span>
                            </h5>
                        </div>
                            ...
                            <div class="dropdown show float-right">
                                <div class="col-auto my-1">
                                    ...
                                    <select class="custom-select mr-sm-2" id="sort_product" name="sort_product">
                                        <option value="1" {{ request()->sort == '1' ? 'selected' : '' }}>
                                            {{ __('messages.sort_by_recommended') }}</option>
                        ...
                        <div class="col-12 d-block d-lg-none px-2">
                            <button type="button" class="float-left" data-toggle="modal" data-target="#exampleModalLong"
                                id="menuModal">
                    ...
                    <div class="row box-List pt-3">
                        ...
                        <div class="col-xl-3 col-lg-3 col-sm-4 col-6 list">
                            <div class="btn-heart {{ $product->favorites_count > 0 ? 'active' : '' }}"
                                onclick="alert('click');"></div>
                            <div class="card h-100">
                                <div class="card-body">
                                    <a
                                        href="{{ route('frontend.product-detail', ['locale' => get_lang(), 'product' => $product->id]) }}">
                                        <div class="img">
                                            <div class="src-img"
                                                style="background-image: url({{ $product->image ?? 'http://via.placeholder.com/500x350' }})">
                                                <img src="{{ asset('images/size-img.png') }}" alt="">
                                                <!-- ช่องนี้ห้ามแก้ -->
                                            </div>
                                        </div>
                                        <div class="box-text">
                                            ...
                                            <span>{{ __('messages.grade') }} -
                                                {{ $product->grades_name->{ get_lang('name') } }}</span>
                                        </div>
                                    </a>
                                </div>
                                <div class="card-footer">
                                    <span class="price">{{ __('messages.price') }} :
                                        ฿{{ number_format($product->price) }}<b>฿{{ number_format($product->full_price) }}</b></span>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form id="form_search_product_mb" class="form-Search"
                    action="{{ route('frontend.product', ['locale' => get_lang()]) }}" method="post">
                    @method('get')
                    @csrf
                    <div class="form-inline">
                        <input type="text" class="form-control" name="keyword" placeholder="ค้นหาสินค้า.."
                    ...
                    <label for="m_category_checkbox{{ $category->id }}" class="d-block">
                        <input type="checkbox" id="m_category_checkbox{{ $category->id }}" name="category_selected[]"
                            value="{{ $category->id }}"
                            {{ (!empty(request()->category_selected) and in_array($category->id, request()->category_selected)) ? 'checked' : '' }}>
                        {{ $category->{ get_lang('name') } }}
                        <span class="float-right">({{ $category->products_count }})</span>
                    ...
                    <label for="m_grade_checkbox{{ $grade->id }}" class="d-block">
                        <input type="checkbox" id="m_grade_checkbox{{ $grade->id }}" name="grade_selected[]"
                            value="{{ $grade->id }}"
                            {{ (!empty(request()->grade_selected) and in_array($grade->id, request()->grade_selected)) ? 'checked' : '' }}>
                        {{ $grade->{ get_lang('name') } }}
                        <span class="float-right">({{ $grade->products_count }})</span>
