        ...
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a
                            href="{{ route('frontend.product', ['locale' => get_lang()]) }}">{{ $pages->{ get_lang('title') } }}</a>
                    </li>
        ...
        <div class="container">
            <div class="row" id="Boxheight">
                    ...
                    <div class="row">
                        <div class="col-md-6 box-showImg">
                            <div class="img d-none d-md-block">
                                <div class="src-img"
                                    style="background-image: url({{ $product->image ?? 'http://via.placeholder.com/500x350' }})"
                                    id="showImg2">
                                ...
                                $i++;
                                @endphp
                                <a href="{{ $img->getUrl() ?? 'http://via.placeholder.com/500x350' }}"
                                    data-fancybox="images" data-id="{{ $i }}" class="{{ $i == 1 ? 'active' : '' }}"></a>
                                @endforeach
                                @endif
                            </div>
                            <div class="owl-carousel">
                                ...
                                <div class="item img">
                                    <a href="javascript:;" onclick="showImg({{ $i }}, '{{ $img->getUrl() }}');">
                                        <div class="src-img"
                                            style="background-image: url({{ $img->getUrl() ?? 'http://via.placeholder.com/500x350' }})">
                                            <img src="{{ asset('images/size-img2.png') }}" alt="">
                                            <!-- ช่องนี้ห้ามแก้ -->
                                        </div>
                                    </a>
                                </div>
                        ...
                        <div class="col-md-6">
                            <h4 class="text-head">
                                <!-- favorites mobile -->
                                <div class="icon-Favorites float-right ml-2 d-block d-lg-none btn-heart {{ $product->favorites_count > 0 ? 'active' : '' }}"
                                    data-fav="{{ $product->favorites_count }}" data-product="{{ $product->id }}">
                                    {{ __('messages.favorite') }}</div>
                                <!-- favorites mobile -->
                            ...
                            <button type="button" id="button-Toggle1" class="p-2 mt-2 border-0 w-100"
                                onclick="$('#b-Toggle').toggleClass('open');$(this).hide();$('#button-Toggle2').show();">
                                {{ __('messages.show_more') }} <img src="{{ asset('images/icon-select.png') }}">
                            </button>
                            <button type="button" id="button-Toggle2" class="p-2 mt-2 border-0 w-100"
                                style="display: none"
                                onclick="$('#b-Toggle').toggleClass('open');$(this).hide();$('#button-Toggle1').show();">
                                {{ __('messages.show_less') }} <img id="button-Toggle2"
                                    src="{{ asset('images/icon-select.png') }}" style="transform: rotate(180deg);">
                            </button>
                ...
                <div class="col-lg-3 box-left-sticky">
                    <div class="b-sticky">
                        <!-- favorites PC -->
                        <div class="icon-Favorites d-none d-lg-block btn-heart {{ $product->favorites_count > 0 ? 'active' : '' }}"
                            data-fav="{{ $product->favorites_count }}" data-product="{{ $product->id }}">
                            {{ __('messages.favorite') }}</div>
                        <!-- favorites PC -->

                        <hr class="d-none d-lg-block">
                        <div class="font-weight-bold mt-3 clearfix">{{ __('messages.grade') }} <h3 class="float-right">
                                {{ $product->grades_name->{ get_lang('name') } }}</h3>
                        </div>
                        <div class="font-weight-bold mt-3 clearfix"><b
                                class="line-through">฿{{ number_format($product->full_price) }}</b>
                        ...
                        <div class="my-3 d-none d-xl-block" style="color: #00b16b;">{{ __('messages.share') }} :
                            <div class="box-social d-inline-block">
                                <a class="I-fb" href="{{ get_link_share_facebook() }}" target="_blank" rel="noopener"
                                    aria-label="facebook"><img src="{{ asset('images/icon-social-fb.png') }}"></a>
                                <a class="I-tw" href="{{ get_link_share_twitter( $product->{ get_lang('title') } ) }}"
                                    target="_blank" rel="noopener" aria-label="twitter"><img
                                        src="{{ asset('images/icon-social-tw.png') }}"></a>
                            </div>
                        </div>
                        <!-- social PC -->
                    </div>
                ...
                <div class="col-lg-9">
                    ...
                    <button type="button" id="button-Toggle3" class="p-2 mt-2 border-0 w-100"
                        onclick="$('#b-Toggle2').toggleClass('open');$(this).hide();$('#button-Toggle4').show();">
                        {{ __('messages.show_more') }} <img src="{{ asset('images/icon-select.png') }}">
                    </button>
                    <button type="button" id="button-Toggle4" class="p-2 mt-2 border-0 w-100" style="display: none"
                        onclick="$('#b-Toggle2').toggleClass('open');$(this).hide();$('#button-Toggle3').show();">
                        {{ __('messages.show_less') }} <img id="button-Toggle2"
                            src="{{ asset('images/icon-select.png') }}" style="transform: rotate(180deg);">
                    </button>
                    <hr class="mt-4">
            ...
            <div class="owl-carousel box-List">
                ...
                <div class="card item col-12 list">
                    <div class="btn-heart {{ $new_product->favorites_count > 0 ? 'active' : '' }}"
                        data-fav="{{ $new_product->favorites_count }}" data-product="{{ $new_product->id }}"></div>
                    <div class="card-body">
                        <a
