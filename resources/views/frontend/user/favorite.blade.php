    ...
    <section class="box-products mt-2">
            ...
            <div class="dropdown show float-right w-100">
              <div class="col-auto my-1">
                ...
                <form id="form_search_product" class="form-Search" action="{{ route('frontend.user.favorite', ['locale' => get_lang()]) }}" method="post">
                  @method('get')
                  @csrf
          ...
          @foreach ($favorites as $favorite)
            <div class="col-xl-2 col-lg-3 col-sm-4 col-6 list">
              ...
              <div class="card h-100">
                <div class="card-body">
                  <a href="{{ route('frontend.product-detail', ['locale' => get_lang(), 'product' => $favorite->products_id]) }}">
                    <div class="img">
                      <div class="src-img" style="background-image: url({{ $favorite->products->image ?? 'http://via.placeholder.com/500x350' }})">
                        <img src="{{ asset('images/size-img.png') }}" alt=""><!-- ช่องนี้ห้ามแก้ -->
                      </div>
                    </div>
                    <div class="box-text">
                      <h6>{{ $favorite->products->{ get_lang('name') } }}</h6>
                      <span>{{ __('messages.grade') }} - {{ $favorite->products->grades_name->{ get_lang('name') } }}</span>
                    </div>
                  </a>
                </div>
                <div class="card-footer">
                  <span class="price">{{ __('messages.price') }} : ฿{{ number_format($favorite->products->product_price) }}<b>฿{{ number_format($favorite->products->full_price) }}</b></span>
