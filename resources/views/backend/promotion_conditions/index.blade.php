    ...
    <div class="col-12 col-xl-12">
        <div class="panel panel-inverse gray">
            <div class="panel-body mgbt">
                <form id="" action="{{ route('backend.promotion_conditions.index') }}" method='post'
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-lg-6 col-md-12 col-sm-12">
                            <div class='mt-4 '>
                                ...
                                @can('add promotions')
                                <a href="{{ route('backend.promotion_conditions.create') }}"
                ...
                <div class="panel-body">
                    <table id="data-table-list" class="table table-striped table-bordered w-100 nowrap">
                        ...
                        <tbody>
                            ...
                            @foreach($promotion_conditions as $promotion_condition)
                            <tr class="del">
                                <td class="text-center">
                                    <div class=" dropright">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            ...
                                            <form
                                                action="{{ route('backend.promotion_conditions.destroy', ['promotion_condition' => $promotion_condition->id]) }}"
                                                method="post">
                                                {{ method_field('DELETE') }}
                                            ...
                                            <a href="{{ route('backend.promotion_conditions.edit', ['promotion_condition' => $promotion_condition->id]) }}"
                                ...
                                <td class="text-center">
                                    {{ date('d/m/Y H:i:s', strtotime($promotion_condition->updated_at)) }}
                                </td>
                                <td class="text-left">
                                    {{ $promotion_condition->promotions->name_th.' ('.$promotion_condition->promotions->name_en.')' }}
                                </td>
                                <td class="text-center">
                                    {{ $promotion_condition->promotion_types->name  }}
                                </td>
                                <td class="text-center">
                                    {{ date('d/m/Y', strtotime($promotion_condition->promotions->start_at)) }}</td>
                                <td class="text-center">
                                    {{ date('d/m/Y', strtotime($promotion_condition->promotions->end_at)) }}</td>
                                {{-- <td class="text-left">{{ $promotion->products->name_th }}</td>
                                <td class="text-right">{{ $promotion->price }}</td> --}}
                                <td class="text-center">{{ $promotion_condition->update_name->first_name }}</td>
                            </tr>
                            @endforeach
                            @endif
                        </tbody>
