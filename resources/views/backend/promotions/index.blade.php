    ...
    <div class="col-12 col-xl-12">
        <div class="panel panel-inverse gray">
            <div class="panel-body mgbt">
                <form id="" action="{{ route('backend.promotions.index') }}" method='post' data-parsley-validate="true">
                    @method('get')
                    @csrf
                    <div class="form-row">
                        ...
                        <div class="form-group col-lg-6 col-md-12 col-sm-12">
                            <div class='mt-4 '>
                                ...
                                @can('add promotions')
                                <a href="{{ route('backend.promotions.create') }}" class="btn btn-white btn-search"><i
                    ...
                    <table id="data-table-list" class="table table-striped table-bordered w-100 nowrap">
                        ...
                        <tbody>
                            ...
                            <tr class="del">
                                <td class="text-center">
                                    <div class=" dropright">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            ...
                                            <form
                                                action="{{ route('backend.promotions.destroy', ['promotion' => $promotion->id]) }}"
                                                method="post">
                                                {{ method_field('DELETE') }}
                                            ...
                                            <a href="{{ route('backend.promotions.edit', ['promotion' => $promotion->id]) }}"
                                ...
                                <td class="text-center">{{ date('d/m/Y H:i:s', strtotime($promotion->updated_at)) }}
                                </td>
                                <td class="text-left">{{ $promotion->name_th }}</td>
                                <td class="text-left">{{ $promotion->name_en }}</td>
                                <td class="text-center">{{ date('d/m/Y', strtotime($promotion->start_at)) }}</td>
                                <td class="text-center">{{ date('d/m/Y', strtotime($promotion->end_at)) }}
                                </td>
                                <td class="text-center">{{ $promotion->active }}</td>
                                <td class="text-center">{{ $promotion->update_name->first_name }}</td>
                            </tr>
                            @endforeach
                            @endif
                        </tbody>
