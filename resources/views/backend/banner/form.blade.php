  ...
  <div class="col-md-12">
      ...
      <div class="col-md-12">
        <div class="row">
          <div class="col-xl-3 col-md-12 col-sm-12">
            <label class='col-form-label w-100' for="end_date">หน้า <span class='text-danger'> * </span> :  </label>
            <select name="page_id" id="page_id" class='form-control select-2'  required {{ !empty($banner->id) ? 'disabled' : ""}}>
            @foreach($pages as $page)

              <option value="{{ $page->id }}" {{ !empty($banner->page_id) && $banner->page_id == $page->id ? 'selected' : '' }}>{{ $page->name}}</option>
            @endforeach
            </select>
          </div>
          <div class="col-xl-3 col-md-12 col-sm-12">
            <label class='col-form-label' for="name">ประเภทแบนเนอร์ <span class='text-danger'> * </span> : </label>
             <select name="type" id="type" class='form-control'  required='' {{ !empty($banner->id) ? 'disabled' : ""}}>
              @foreach($types as $type)
                <option value="{{ $type }}" {{ !empty($banner->type) && $banner->type == $type ? 'selected' : '' }}>{{ $type }}</option>
              @endforeach
            </select>
          </div>
          <div class="col-xl-3 col-md-12 col-sm-12">
            <label class='col-form-label' for="position">ตำแหน่งแบนเนอร์ <span class='text-danger'> * </span> :  </label>
            <select name="position" id="position" class='form-control'  required {{ !empty($banner->id) ? 'disabled' : ""}}>
              @foreach($positions as $position)

                <option value="{{ $position }}" {{ !empty($banner->position) && $banner->position == $position ? 'selected' : '' }}>{{ $position}}</option>
              @endforeach
            </select>
          </div>
        </div>
      ...
      <div class="col-md-12">
        <div class="row">
          <div class="col-xl-6 col-md-12 col-sm-12">
            <label class='col-form-label' for="name">ชื่อแบนเนอร์ <span class='text-danger'> * </span> : </label>
    ...
    @if(empty($banner->id))
              ...
              <div class="card border">
                <div class="card-body">
                  <div class="row form-silde">
                    <div class="col-xl-3 col-md-12 col-sm-12">
                      <label class='col-form-label' for="static_type">ประเภท <span class="text-danger"> * </span> :  </label>
                    ...
                    <div class="col-xl-3 col-md-12 col-sm-12 slide-pic">
                      <label class="col-form-label " for="image">รูป Pc<span class="text-danger"> * </span> :  </label>
                    ...
                    <div class="col-xl-3 col-md-12 col-sm-12 slide-pic">
                      <label class="col-form-label " for="image">รูป Mobile<span class="text-danger"> * </span> :  </label>
      ...
      <div class="row mt-3 mr-1 dynamic" style='display: none;'>
        <div class="col-md-12">
          <div class="row">
            <button class='btn btn-success add-slide ml-2'> <i class='fas fa-plus'></i> เพิ่มสไลด์แบนเนอร์</button>
          </div>
        </div>
      </div>
                    ...
                    <div class="row form-silde">
                      <div class="col-xl-2 col-md-12 col-sm-12">
                        ...
                        <select name="edit_type" id="static_type" class='form-control slide_type'  required>
                          ...
                          {{-- <option value="video" {{$banner_detail->type == 'video' ? 'selected' : ''}}>video</option> --}}
                      ...
                      <div class="col-xl-3 col-md-12 col-sm-12">
                       ...
                       <div class="col-xl-3 col-md-12 col-sm-12 slide-pic">
                        <label class="col-form-label" for="image">รูป Pc<span class="text-danger"> * </span> :  </label>
                        <input type="file" id='{{$banner_detail->id}}' name="edit_upload_pc" class="image form-control mt-2" >
                        <label class="text-pic">ขนาดภาพที่แนะนำ 1920x768 (ขนาดไม่เกิน 5 MB)</label>
                        <div class="d-flex justify-content-center">
                          <img src="{{$banner_detail->slide_banner_pc}}" class="img-thumbnail m-4 img-preview" id='preview-{{$banner_detail->id}}'>
                        </div>
                      </div>
                      ...
                      <div class="col-xl-3 col-md-12 col-sm-12 slide-pic">
                        <label class="col-form-label" for="image">รูป Mobile<span class="text-danger"> * </span> :  </label>
                        <input type="file" id='{{$banner_detail->id}}' name="edit_upload_mobile" class="image_mobile form-control mt-2" >
                        <label class="text-pic">ขนาดภาพที่แนะนำ 1920x768 (ขนาดไม่เกิน 5 MB)</label>
                        <div class="d-flex justify-content-center">
                          <img src="{{$banner_detail->slide_banner_mobile}}" class="img-thumbnail m-4 img-preview" id='preview-mobile-{{$banner_detail->id}}'>
                        </div>
                      </div>
                ...
                <div class="card border">
                  <div class="card-body">
                    <div class="row form-silde">
                      ...
                      <div class="col-xl-3 col-md-12 col-sm-12 side-url">
                        <label class='col-form-label ' for="static_type">Link <span class='text-danger'> * </span> :  </label>
                        <input type="text" name='edit_url[]' class='slide_url form-control mt-2' value='{{$banner_detail->url}}'>
                      </div>

                      @if($banner_detail->type == 'image')
                        <div class="col-xl-3 col-md-12 col-sm-12 slide-pic">
                          <label class="col-form-label " for="edit_slide_upload">รูป PC <span class="text-danger"> * </span> :  </label>
                          <input type="file" id='{{$banner_detail->id}}' name="edit_upload_pc[{{$key}}]" class="edit_slide_upload form-control mt-2 image" >
                          <label class="text-pic">ขนาดภาพที่แนะนำ 1920x768 (ขนาดไม่เกิน 5 MB)</label>
                          <div class="d-flex justify-content-center">
                            <img src="{{$banner_detail->slide_banner_pc}}" class="img-thumbnail m-4 img-preview" id='preview-{{$banner_detail->id}}'>
                          </div>
                         ...
                         <div class="col-xl-3 col-md-12 col-sm-12 slide-pic">
                          <label class="col-form-label " for="edit_slide_upload">รูป Mobile <span class="text-danger"> * </span> :  </label>
                          <input type="file" id='{{$banner_detail->id}}' name="edit_upload_mobile[{{$key}}]" class="edit_slide_upload form-control mt-4 image_mobile" >
                          <label class="text-pic">ขนาดภาพที่แนะนำ 1920x768 (ขนาดไม่เกิน 5 MB)</label>
                          <div class="d-flex justify-content-center">
                            <img src="{{$banner_detail->slide_banner_mobile}}" class="img-thumbnail m-4 img-preview" id='preview-mobile-{{$banner_detail->id}}'>
                          </div>
