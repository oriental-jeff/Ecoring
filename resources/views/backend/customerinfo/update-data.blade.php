...
@section('title')
<i class="fad fa-lg fa-user"></i> แก้ไขข้อมูลลูกค้า : {{ $user->first_name }} {{ $user->last_name }}
@endsection
        ...
        <div class="panel" data-sortable-id="form-validation-1">
            <div class="panel-body">
                <form class="form-horizontal" id="form-validate" name="demo-form" enctype="multipart/form-data"
                    action="{{ route('backend.customerinfo.update', ['customerinfo' => $user->id] ) }}" method='post'>
                    @method('PATCH')
                    @csrf
                    @include('backend.customerinfo.form-update-data')
                </form>
            </div>
            <!-- end panel-body -->
        </div>
...
<script>
    ...
    function deactivated(user_id) {
        window.location.replace('/backend/customerinfo/deactivation_user/' + user_id);
    }
